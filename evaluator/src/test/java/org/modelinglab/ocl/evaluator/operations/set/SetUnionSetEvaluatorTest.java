 ...
 *
 */
public class SetUnionSetEvaluatorTest extends AbstractBinaryOperationTest.AbstractCollectionBinaryOperationTest {
    
    public SetUnionSetEvaluatorTest() throws Exception {
    ...
    public void testVisit_SetValue() throws Exception {
        ...
        executeTest(input, TesterTool.evaluate("Set{1, 2, 2, 3}", null, false), TesterTool.evaluate("Set{1, 2, 3, 0.0}->excluding(0.0)", null, false));
        executeTest(input, TesterTool.evaluate("Set{2, 2, 3}", null, false), TesterTool.evaluate("Set{1, 2, 3, 0.0}->excluding(0.0)", null, false));
        executeTest(input, TesterTool.evaluate("Set{1, 2}", null, false), TesterTool.evaluate("Set{2, 1, 3, 0.0}->excluding(0.0)", null, false));
        executeTest(input, TesterTool.evaluate("Set{10, 11, 12, 12}", null, false), TesterTool.evaluate("Set{1, 2, 3, 10, 11, 12, 12, 0.0}->excluding(0.0)", null, false));
    }
